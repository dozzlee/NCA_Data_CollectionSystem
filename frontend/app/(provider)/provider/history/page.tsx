...
export default function ProviderHistoryPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="overflow-x-auto rounded-2xl border bg-white"><table className="w-full min-w-[840px] text-left"><thead className="border-b bg-[#f7f9fb]"><tr>{["Form", "Period", "Submitted", "Responsible approver", "Status", "Receipt", ""].map((h) => <th key={h} className="px-5 py-3 text-xs font-semibold uppercase text-[#43474f]">{h}</th>)}</tr></thead>
      ...
      : data.results.map((row) => <tr key={row.id}><td className="px-5 py-4"><p className="text-sm font-medium">{row.form_name}</p><p className="font-mono text-xs text-[#737780]">{row.form_code}</p>{row.submission_reference&&<p className="mt-1 max-w-[260px] break-all font-mono text-[10px] text-[#004999]">{row.submission_reference}</p>}</td><td className="px-5 py-4 text-sm">{row.period_name}</td><td className="px-5 py-4 text-sm text-[#737780]">{row.submitted_at ? new Date(row.submitted_at).toLocaleString() : "—"}</td><td className="px-5 py-4 text-sm">{row.last_edited_by_name || "—"}</td><td className="px-5 py-4"><WorkflowBadge status={row.workflow_status} /></td><td className="px-5 py-4 text-sm">{row.receipt_available ? row.receipt_reference : "—"}</td><td className="px-5 py-4"><Link href={`/provider/submissions/${row.id}`} className="text-sm font-medium text-[#0066cc]">View →</Link></td></tr>)}</tbody>
    </table></div>
  </div>;
}
