  ...
  return <div className="space-y-6">
    ...
    : <div className="space-y-3">{data.results.map((row) => <article key={row.id} className="rounded-2xl border bg-white p-5">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div><div className="flex flex-wrap items-center gap-2"><WorkflowBadge status={row.workflow_status} /><DueStateBadge state={row.due_state} /></div><h2 className="mt-2 font-semibold">{row.form_name}</h2><p className="text-sm text-[#737780]">{row.period_name} · {Number(row.completion_pct).toFixed(0)}% complete · {row.open_correction_count} open corrections</p>{row.submission_reference&&<p className="mt-1 break-all font-mono text-[10px] text-[#004999]">{row.submission_reference}</p>}<p className="mt-2 text-xs text-[#43474f]">Last edited by {row.last_edited_by_name || "Data Entry"}{row.last_edited_at ? ` on ${new Date(row.last_edited_at).toLocaleString()}` : ""}</p></div>
