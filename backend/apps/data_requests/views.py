...
from apps.users.permissions import IsDataRequester, IsNCAEditor, IsNCAUser
...
from .services import build_manifest, store_delivery_attachment, upload_artifact
...
def catalog(request):
    ...
    for form in forms:
        ...
        eligible = ExpectedSubmission.objects.filter(form_template=form, workflow_status="APPROVED")
        period_ids = list(eligible.values_list("period_id", flat=True).distinct())
        applicable_provider_ids = list(eligible.values_list("provider_id", flat=True).distinct())
        provider_periods = {}
        for period_id, provider_id in eligible.values_list("period_id", "provider_id"):
            provider_periods.setdefault(str(period_id), []).append(provider_id)
        form_items.append({"id": form.id, "code": form.form_code, "name": form.name, "description": form.instructions,
            ...
            "available_period_ids": period_ids, "applicable_provider_ids": applicable_provider_ids,
            "applicable_provider_ids_by_period": provider_periods, "sections": sections})
...
class RequestListCreate(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.role in {"NCA_ADMIN", "NCA_OFFICER"}:
    ...
    def get_permissions(self):
        return [(IsDataRequester if self.request.method == "POST" else (IsNCAUser if self.request.user.role in {"NCA_ADMIN", "NCA_OFFICER"} else IsDataRequester))()]
...
class RequestDetail(generics.RetrieveUpdateAPIView):
    ...
    def get_queryset(self):
        qs = DataRequest.objects.select_related("requester", "reviewer", "artifact").prefetch_related("events")
        return qs if self.request.user.role in {"NCA_ADMIN", "NCA_OFFICER"} else qs.filter(requester=self.request.user)
    def get_permissions(self): return [(IsNCAUser if self.request.user.role in {"NCA_ADMIN", "NCA_OFFICER"} else IsDataRequester)()]
...
def decision(request, pk, target, event, require_note=False):
    ...
    uploaded = request.FILES.get("file") if target == "APPROVED" else None
    delivery_mode = request.data.get("delivery_mode") or ("ATTACHMENT_ONLY" if uploaded else "DATASET_ONLY")
    if target == "APPROVED" and delivery_mode not in {"DATASET_ONLY", "ATTACHMENT_ONLY", "DATASET_AND_ATTACHMENT"}:
        return Response({"detail": "Choose a valid delivery option."}, status=400)
    if target == "APPROVED" and delivery_mode in {"ATTACHMENT_ONLY", "DATASET_AND_ATTACHMENT"} and not uploaded:
        return Response({"detail": "Attach a file for the selected delivery option."}, status=400)
    if uploaded and delivery_mode == "ATTACHMENT_ONLY":
        try:
            artifact = upload_artifact(item, uploaded, request.user, enforce_requested_format="delivery_mode" not in request.data)
        except (ValueError, OSError) as exc:
            return Response({"detail": str(exc)}, status=400)
        item.status = "READY"
        item.approved_at = item.completed_at = timezone.now()
        item.approval_manifest = {"source": "ADMIN_UPLOAD", "filename": artifact.filename, "sha256": artifact.sha256}
        item.save()
        add_event(item, request.user, "FILE_RELEASED", old, "READY", note or "Your requested extract is ready to download.",
            {"filename": artifact.filename, "sha256": artifact.sha256}, notify=True)
        return Response(DataRequestSerializer(DataRequest.objects.select_related("artifact").get(pk=item.pk)).data)
    if target == "APPROVED":
        try: item.approval_manifest=build_manifest(item)
        except ValueError as exc: return Response({"detail": str(exc)}, status=400)
        item.approval_manifest["delivery_mode"] = delivery_mode
        if uploaded:
            try: item.approval_manifest["delivery_attachment"] = store_delivery_attachment(item, uploaded)
            except (ValueError, OSError) as exc: return Response({"detail": str(exc)}, status=400)
        item.approved_at=timezone.now()
    item.save(); add_event(item, request.user, event, old, target, note or event.replace("_", " ").title(), notify=True)
    if target == "APPROVED":
        transaction.on_commit(lambda: generate_data_request.delay(str(item.id)))
...
def resubmit(request, pk):
    ...
    old=item.status; item.status="SUBMITTED"; item.decision_note=""; item.save(); add_event(item, request.user, "RESUBMITTED", old, item.status, "Updated request resubmitted.", notify_admins=True)
    return Response(DataRequestSerializer(item).data)
...
def withdraw(request, pk):
    ...
    old=item.status; item.status="WITHDRAWN"; item.save(); add_event(item, request.user, "WITHDRAWN", old, item.status, "Request withdrawn.", notify_admins=True)
    return Response(DataRequestSerializer(item).data)
...
def retry(request, pk):
    item=get_object_or_404(DataRequest.objects.select_for_update(), pk=pk)
    if item.status != "GENERATION_FAILED": return Response({"detail": "Only failed files can be retried."}, status=409)
    old = item.status
    item.status = "PREPARING"
    item.save(update_fields=["status", "updated_at"])
    add_event(item, request.user, "GENERATION_RETRIED", old, item.status, "File generation retry started.", notify=True)
    transaction.on_commit(lambda: generate_data_request.delay(str(item.id)))
    return Response({"detail": "Generation restarted.", "status": item.status}, status=202)


@api_view(["GET"])
def download(request, pk):
    item=get_object_or_404(DataRequest.objects.select_related("artifact"), pk=pk)
    if request.user.role not in {"NCA_ADMIN", "NCA_OFFICER"} and item.requester_id != request.user.id: return Response({"detail": "Not found."}, status=404)
...
def notifications(request):
    if request.user.role not in {"NCA_VIEWER", "NCA_ADMIN", "NCA_OFFICER"}: return Response({"detail": "Not found."}, status=404)
